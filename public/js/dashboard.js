...
(function () {
  ...
  function buildPolicyForm(container, p, dbs) {
    const v = p || {
      ...
      compression: 'MEDIUM', parallelDegree: 1, content: 'ALL',
    ...
    container.innerHTML = `
      ...
      <div class="row-split">
        <div class="field"><label>압축 (COMPRESSION)</label>
          <select id="f_compression">${['NONE', 'BASIC', 'LOW', 'MEDIUM', 'HIGH'].map((c) => `<option value="${c}" ${c === v.compression ? 'selected' : ''}>${c}</option>`).join('')}</select>
        </div>
