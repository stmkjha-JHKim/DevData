  ...
  // --- Help tab: click-to-enlarge screenshots ---
  // A minimal, dependency-free lightbox: click any .help-shot image to see
  // it at near-full-window size (small print in a narrow window is
  // otherwise hard to read); click the overlay, press Escape, or click the
  // image again to close. Built once and reused for every screenshot in
  // both language blocks.
  const helpLightbox = document.getElementById('helpLightbox');
  const helpLightboxImg = document.getElementById('helpLightboxImg');
  function openHelpLightbox(src, alt) {
    if (!helpLightbox || !helpLightboxImg) return;
    helpLightboxImg.src = src;
    helpLightboxImg.alt = alt || '';
    helpLightbox.classList.add('open');
  }
  function closeHelpLightbox() {
    if (!helpLightbox) return;
    helpLightbox.classList.remove('open');
    helpLightboxImg.src = '';
  }
  document.querySelectorAll('.help-shot img').forEach((img) => {
    img.addEventListener('click', () => openHelpLightbox(img.getAttribute('src'), img.getAttribute('alt')));
  });
  if (helpLightbox) {
    helpLightbox.addEventListener('click', closeHelpLightbox);
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') closeHelpLightbox();
    });
  }
  ...
  function applySessionFilters() {
    ...
    const sidInput = document.getElementById('filterSid');
    ...
    // Free-text substring match rather than an exact-value dropdown (unlike
    // Status/Machine/Program): SID is a per-session number that's almost
    // always unique and constantly churns as sessions log on/off, so a
    // dropdown of "every current SID" would be unusable -- typing part of
    // the number you're looking for is what's actually useful here.
    const sidFilter = sidInput.value.trim();
    ...
    const filtered = section.data.filter(row => {
      ...
      const sidOk = !sidFilter || String(row.SID != null ? row.SID : '').includes(sidFilter);
      return statusOk && machineOk && programOk && sidOk;
    });
    ...
    const count = (statusFilter || machineFilter || programFilter || sidFilter)
    ...
    box.innerHTML = `
      ...
      <div class="sess-table-wrap">
        <table class="sess-table">
          <thead>
            <tr>
              <th>SID</th><th>SERIAL#</th><th>${t('statusHeader')}</th><th>${t('machineHeader')}</th><th>${t('osUserHeader')}</th><th>${t('programHeader')}</th><th>${t('logonTimeHeader')}</th><th>SQL_ID</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
    ...
    const rows = section.data.map(row => `
      <tr>
        ...
        <td class="ts-meter-cell">
          ${row.USED_PCT != null
            ? `<span class="ts-meter-pct">${Number(row.USED_PCT).toLocaleString()}%</span>${renderMeter(row.USED_PCT)}`
            : '-'}
        </td>
      </tr>`).join('');
    box.innerHTML = `
      <div class="sess-table-wrap">
        <table class="sess-table">
          <thead><tr>
            <th>${t('tablespaceHeader')}</th><th>${t('usedHeader')}</th><th>${t('freeHeader')}</th><th>${t('totalHeader')}</th><th>${t('maxSizeHeader')}</th><th>${t('usedPctOfMaxHeader')}</th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
