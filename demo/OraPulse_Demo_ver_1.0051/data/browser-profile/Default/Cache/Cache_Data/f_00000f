  // Demo mode: flipped on by /api/version's demoMode flag (backed by a
  // DEMO marker file only build-demo.ps1's packaging step drops next to
  // the .exe -- see backend/core.py's IS_DEMO_MODE). false until that
  // fetch resolves, so nothing is restricted for the brief moment before
  // it's known either way. See applyDemoModeUi() below for what it turns off.
  let isDemoMode = false;
  const DEMO_DISABLED_TABS = ['recovery', 'tuning', 'objectview'];
  ...
  function switchTab(tab) {
    if (!TABS[tab]) return;
    if (isDemoMode && DEMO_DISABLED_TABS.includes(tab)) return;
  ...
  function openTableStatsCtxMenu(x, y, owner, tableName, partitionName) {
    ...
    tableStatsCtxMenuTitle.textContent = partitionName
      ...
      : `${owner}.${tableName}`;
    tableStatsCtxMenuGather.disabled = isDemoMode;
    tableStatsCtxMenuGather.textContent = t('btnGatherStats');
    tableStatsCtxMenuGather.title = isDemoMode ? t('demoDisabledTooltip') : '';
  ...
  tableStatsCtxMenuGather.addEventListener('click', async () => {
    if (!ctxTarget || isDemoMode) return;
  ...
  function updateTableStatsBatchUI() {
    ...
    if (batchBtn) {
      batchBtn.disabled = isDemoMode || checked.length === 0;
      if (isDemoMode) batchBtn.title = t('demoDisabledTooltip');
    }
  ...
  (async function loadAppVersion() {
    try {
      ...
      // Non-critical -- just leave the version text empty (and demo mode off).
    }
  })();

  // Disables everything the demo build restricts, once /api/version has
  // confirmed demoMode is on: every tab except DashBoard/Ops (switchTab()
  // above already refuses to switch into one of these, this just makes
  // that visible), and the Table Statistics Collection card's gather-stats
  // actions (the single-row context menu item and the batch button). The
  // session list's right-click menu is guarded separately, directly inside
  // openSessionCtxMenu() below, since it's rebuilt fresh on every open
  // rather than sitting in the DOM to be disabled once here.
  function applyDemoModeUi() {
    DEMO_DISABLED_TABS.forEach(key => {
      const btn = TABS[key] && TABS[key].btn;
      if (!btn) return;
  ...
  // Clicking Report asks for a filename first (client-side prompt(), so the
  // event order is exactly "click -> name the file -> then generate"), then
  // POSTs to /api/generate-report. The server saves the generated HTML
  // straight to disk next to the running app (report/<YYYY-MM-DD>/<name>)
  // and replies with where it landed -- there is no browser download step
  // here at all, since that depended on the browser's own download/Save-As
  // handling actually completing, which wasn't reliable.
  function defaultReportFilename() {
    const d = new Date();
    const pad = (n) => String(n).padStart(2, '0');
    return `orapulse-report-${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}.html`;
  }

  reportBtn.addEventListener('click', async () => {
    let filename = window.prompt(t('reportFilenamePrompt'), defaultReportFilename());
    if (filename === null) return; // user cancelled the prompt
    filename = filename.trim() || defaultReportFilename();
    ...
    try {
      const res = await fetch('/api/generate-report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ days: 7, lang: currentLang, filename })
      });
      ...
      showToast(t('reportGeneratedToast', { path: data.path }), 'ok');
    } catch (err) {
  ...
  function openSessionCtxMenu(x, y, sid, serial, sqlId) {
    // The demo build disables this menu entirely (both "View Running
    // Query" and "Kill Session"), rather than disabling each item --
    // left-click's "wait detail" view (openSessionDetail(), a plain read)
    // is unaffected and stays fully usable in demo mode.
